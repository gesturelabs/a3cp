...
- [ ] **ID semantics (critical):** Confirm whether `record_id` is strictly **per-message** (UUIDv4) per A3CPMessage spec.
      If yes, define a separate bounded-capture correlator (`capture_id` or `window_id`) and use it consistently:
      - browser → camera_feed_worker ingress includes `capture_id`
      - camera_feed_worker → landmark_extractor forwards `capture_id`
      - landmark_extractor output references the same `capture_id`
      - each emitted A3CPMessage gets its own unique `record_id`

- [ ] **Authoritative generator for IDs:** Decide where each ID is created:
      - `session_id`: session_manager only
      - `record_id`: generated by the emitting module for each message
      - `capture_id/window_id`: generated by browser OR camera_feed_worker (pick one and remove conflicting wording)
