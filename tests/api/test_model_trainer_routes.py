...
@pytest.mark.anyio
async def test_train_model_returns_501():
    input_payload = load_example("model_trainer", "input")
    # expected_output = load_example("model_trainer", "output")  # Uncomment when 200 OK is returned

    async with LifespanManager(app):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post("/api/model_trainer/", json=input_payload)
