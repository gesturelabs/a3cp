...
async def test_expand_output_phrase_returns_501():
    input_payload = load_example("output_expander", "input")
    # expected_output = load_example("output_expander", "output")  # Uncomment when 200 OK is returned

    async with LifespanManager(app):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post("/api/output_expander/", json=input_payload)
            ...
            #     assert actual[key] == expected_output_
