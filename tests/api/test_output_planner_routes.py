...
@pytest.mark.anyio
async def test_output_planner_stub_returns_501():
    input_payload = load_example("output_planner", "input")
    # expected_output = load_example("output_planner", "output")  # Uncomment when 200 OK is returned

    async with LifespanManager(app):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post("/api/output_planner/", json=input_payload)
