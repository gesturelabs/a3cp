...
@pytest.mark.anyio
async def test_log_feedback_returns_501():
    input_payload = load_example("feedback_log", "input")
    # expected_output = load_example("feedback_log", "output")  # Uncomment when implementing route

    async with LifespanManager(app):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post("/api/feedback_log/", json=input_payload)
